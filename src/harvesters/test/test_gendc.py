...
class TestMultiPart(TestHarvester):
    ...
    def test_gendc_1_parts(self):
        ...
        self.assertEqual(buffer.payload.payload_type, genicam.gentl.PAYLOADTYPE_INFO_IDS.PAYLOAD_TYPE_GENDC)
        ...
        image_rgba = buffer.payload.components[0].parts[0].data.reshape(height_rgba, width_rgba, 4)
        self.assertEqual(image_rgba[:, :, 0].min(), 255, "Wrong minimum value of component 0.0")
        self.assertEqual(image_rgba[:, :, 0].max(), 255, "Wrong maximum value of component 0.0")
        self.assertEqual(image_rgba[:, :, 1].min(), 0, "Wrong minimum value of component 0.1")
        self.assertEqual(image_rgba[:, :, 1].max(), 255, "Wrong maximum value of component 0.1")
        self.assertEqual(image_rgba[:, :, 2].min(), 0, "Wrong minimum value of component 0.2")
        self.assertEqual(image_rgba[:, :, 2].max(), 255, "Wrong maximum value of component 0.2")
        self.assertEqual(image_rgba[:, :, 3].min(), 0, "Wrong minimum value of component 0.3")
        self.assertEqual(image_rgba[:, :, 3].max(), 255, "Wrong maximum value of component 0.3")
    ...
    def test_gendc_5_parts(self):
        ...
        self.assertEqual(buffer.payload.components[0].data_format, 'RGBa8')
        self.assertEqual(buffer.payload.components[1].data_format, 'Coord3D_ABC32f_Planar')
        ...
        self.assertEqual(buffer.payload.components[1].parts[0].data_format, 'Coord3D_A32f')
        self.assertEqual(buffer.payload.components[1].parts[1].data_format, 'Coord3D_B32f')
        self.assertEqual(buffer.payload.components[1].parts[2].data_format, 'Coord3D_C32f')

        height_a32f = buffer.payload.components[1].parts[0].height
        height_a32f = buffer.payload.components[1].parts[0].width
        image_a32f = buffer.payload.components[1].parts[0].data.reshape(height_a32f, height_a32f)
        self.assertEqual(image_a32f[:, :].min(), 0, "Wrong minimum value of component 1")
        self.assertEqual(image_a32f[:, :].max(), 399.0, "Wrong maximum value of component 1")

        self.assertEqual(buffer.payload.components[1].parts[1].data_format, 'Coord3D_B32f')
        height_b32f = buffer.payload.components[1].parts[1].height
        height_b32f = buffer.payload.components[1].parts[1].width
        image_b32f = buffer.payload.components[1].parts[1].data.reshape(height_b32f, height_b32f)
        self.assertEqual(image_b32f[:, :].min(), 0, "Wrong minimum value of component 2")
        self.assertEqual(image_b32f[:, :].max(), 399.0, "Wrong maximum value of component 2")

        self.assertEqual(buffer.payload.components[1].parts[2].data_format, 'Coord3D_C32f')
        height_c32f = buffer.payload.components[1].parts[2].height
        height_c32f = buffer.payload.components[1].parts[2].width
        image_c32f = buffer.payload.components[1].parts[2].data.reshape(height_c32f, height_c32f)
        self.assertEqual(image_c32f[:, :].min(), 0, "Wrong minimum value of component 3")
        self.assertAlmostEqual(image_c32f[:, :].max(), 41.04875, msg="Wrong maximum value of component 3")
